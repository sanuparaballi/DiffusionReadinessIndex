...
Workflow:
1. Load configuration/parameters (e.g., dataset paths, TRI selection, simulation params).
...
6. Calculate all selected Transmission Readiness Indices (TRIs) for all nodes.
   - Baselines (Degree, K-Core, Eigenvector, Betweenness, Closeness)
   - Literature Indices (CI, Gravity, FwLTR-structural, SD_ego)
   - Composite TRIs (WMFS-TRI with various feature sets)
   - Mechanism-Focused TRIs (BPI-TRI)
   - Spectral TRIs (Localized Fiedler, Global Fiedler Comp, Localized Spectral Radius, PPR-API, HeatKernel-API)
7. Perform evaluations:
   - Spearman rank correlations (TRIs vs. ETMs).
...
DEFAULT_CONFIG = {
    ...
    "casflow_data_path": "dataset.txt", # Relative to project root or an absolute path
    "snap_twitter_data_path": "78813.txt",
    "results_dir": "results",
    ...
    "tri_selection": { # Which TRIs to compute and evaluate
        "baselines": True,
        "literature": True,
        "composite_wmfs": True,
        "mechanism_bpi": True,
    ...
    "composite_wmfs_configs": [ # List of WMFS configurations to test
        {
            "name": "WMFS_Simple",
            "features": [
                {"name": "degree_out", "func_module": "baselines", "func_name": "get_degree_centrality", "weight": 0.4, "params": {}},
                {"name": "k_core", "func_module": "baselines", "func_name": "get_k_core_centrality", "weight": 0.3, "params": {}},
                {"name": "sd_ego_1hop", "func_module": "literature_indices", "func_name": "get_structural_diversity_ego", "weight": 0.3, "params": {"radius": 1}}
            ]
        },
        # Add more WMFS configurations here, e.g., including spectral features
        {
            ...
            "features": [
                {"name": "degree_out", "func_module": "baselines", "func_name": "get_degree_centrality", "weight": 0.3, "params": {}},
                {"name": "k_core", "func_module": "baselines", "func_name": "get_k_core_centrality", "weight": 0.2, "params": {}},
                {"name": "loc_fiedler_1hop", "func_module": "spectral_tri", "func_name": "get_localized_fiedler_value", "weight": 0.3, "params": {"k_hop": 1}},
                {"name": "ppr_api_1hop", "func_module": "spectral_tri", "func_name": "get_ppr_api_tri", "weight": 0.2, "params": {"k_sum_hops": 1, "ppr_alpha":0.85}}
            ]
        }
    ],
    "spectral_params": {
...
def run_experiment(config):
    ...
    if config["dataset_type"] == "casflow":
        ...
        all_cascade_events_for_etm = etm_functions.preprocess_cascade_data_for_node_etms(raw_cascade_events, list(graph.nodes()))
        print(f"CasFlow: Graph loaded with {graph.number_of_nodes()} nodes, {graph.number_of_edges()} edges.")
        ...
        if graph.number_of_nodes() == 0:
            ...
            return
        print(f"SNAP Twitter: Graph loaded with {graph.number_of_nodes()} nodes, {graph.number_of_edges()} edges.")
        ...
        for seed_node in nodes_to_seed:
            ...
            count_sim_done +=1
            if count_sim_done % 100 == 0 : print(f"  Simulations done for {count_sim_done}/{len(nodes_to_seed)} seeds.")

        all_cascade_events_for_etm = simulated_cascades_by_seed # This is now {seed_node: list_of_cascade_event_lists}
    ...
    etm_scores['ADT'] = {node: etm_functions.calculate_adt(node_centric_cascades.get(str(node), []), str(node)) for node in all_graph_nodes}
    etm_scores['MCSS'] = {node: etm_functions.calculate_mcss(node_centric_cascades.get(str(node), []), str(node)) for node in all_graph_nodes}
    etm_scores['HNR_k'] = {node: etm_functions.calculate_hnr_k(node_centric_cascades.get(str(node), []), str(node), graph, config["etm_calculation"]["hnr_k_hop"]) for node in all_graph_nodes}
    ...
    # --- 3. Calculate TRIs ---
    print("\n--- 3. Calculating TRIs ---")
    tri_results = {} # { 'TRI_Name': {node: score}, ... }

    # Precompute global spectral info if needed
    if config["tri_selection"]["spectral"] and config["spectral_params"]["precompute_global_fiedler"]:
        print("Precomputing global Fiedler information for spectral TRIs...")
        spectral_tri.precompute_global_fiedler_info(graph)

    if config["tri_selection"]["baselines"]:
        print("Calculating Baseline TRIs...")
        tri_results['Degree (Out)'] = baselines.get_degree_centrality(graph, per_node=True)
        tri_results['K-Core'] = baselines.get_k_core_centrality(graph, per_node=True)
        tri_results['Eigenvector'] = baselines.get_eigenvector_centrality(graph, per_node=True)
        # Betweenness can be very slow, consider sampling for large graphs
        # tri_results['Betweenness'] = baselines.get_betweenness_centrality(graph, per_node=True, k_samples=1000 if graph.number_of_nodes() > 5000 else None)
        # tri_results['Closeness'] = baselines.get_closeness_centrality(graph, per_node=True)
        print("Baselines calculated.")

    if config["tri_selection"]["literature"]:
        print("Calculating Literature TRIs...")
        tri_results['CI (l=2)'] = literature_indices.get_collective_influence(graph, per_node=True, l_dist=2)
        # Gravity can be slow. Use radius or ensure graph is not too large.
        # tri_results['Gravity (r=3)'] = literature_indices.get_gravity_centrality(graph, per_node=True, r_radius=3)
        tri_results['SD_ego (r=1)'] = literature_indices.get_structural_diversity_ego(graph, per_node=True, radius=1)
        if graph.is_directed():
             tri_results['FwLTR_struct (t=0.5*indeg)'] = literature_indices.get_fwltr_structural(graph, per_node=True, uniform_threshold=0.5)
        print("Literature TRIs calculated.")

    if config["tri_selection"]["composite_wmfs"]:
        print("Calculating Composite WMFS-TRIs...")
        for wmfs_conf in config["composite_wmfs_configs"]:
            ...
            for feat_conf in wmfs_conf["features"]:
                ...
                # Simplified:
                if module_name == "baselines": module = baselines
                elif module_name == "literature_indices": module = literature_indices
                elif module_name == "spectral_tri": module = spectral_tri
                else: raise ValueError(f"Unknown module: {module_name}")
            ...
            tri_results[name] = composite_tri.get_wmfs_tri(graph, parsed_feature_configs, per_node=True)
            print(f"{name} calculated.")

    if config["tri_selection"]["mechanism_bpi"]:
        print("Calculating BPI-TRI...")
        tri_results['BPI-TRI (norm)'] = mechanism_focused_tri.get_bpi_tri(graph, per_node=True, normalize_components=True)
        print("BPI-TRI calculated.")

    if config["tri_selection"]["spectral"]:
        print("Calculating Spectral TRIs...")
        sp = config["spectral_params"]
        tri_results['LocFiedler (k=1)'] = spectral_tri.get_localized_fiedler_value(graph, per_node=True, k_hop=sp["k_hop_local_spectral"])
        if sp["precompute_global_fiedler"]:
            tri_results['GlobFiedlerComp'] = spectral_tri.get_node_global_fiedler_component(graph, per_node=True)
        tri_results['LocSpecRadius (k=1)'] = spectral_tri.get_localized_spectral_radius(graph, per_node=True, k_hop=sp["k_hop_local_spectral"])
        if graph.is_directed(): # PPR and Heat Kernel are often on DiGraphs for flow
            tri_results[f'PPR-API (k_sum={sp["ppr_k_sum_hops"]})'] = spectral_tri.get_ppr_api_tri(graph, per_node=True, k_sum_hops=sp["ppr_k_sum_hops"], ppr_alpha=sp["ppr_alpha"])
            ...
            #    tri_results[f'HeatKernel-API (t={sp["heat_kernel_t"]})'] = spectral_tri.get_heat_kernel_api_tri(graph, per_node=True, t_short=sp["heat_kernel_t"])
            # else:
            #    print(f"Skipping Heat Kernel TRI for {graph_name} due to size ({graph.number_of_nodes()} nodes).")
        print("Spectral TRIs calculated.")

    if config["save_results_csv"]:
        for tri_name, scores_dict in tri_results.items():
            safe_tri_name = tri_name.replace(" ", "_").replace("(", "").replace(")", "").replace("=","").replace("*","").replace(".","p")
            pd.DataFrame(list(scores_dict.items()), columns=['node', tri_name]).to_csv(
                os.path.join(config["results_dir"], f"{graph_name}_{safe_tri_name}_scores.csv"), index=False)
    print("All TRIs calculated.")
    ...
    for etm_name, current_etm_scores in etm_scores.items():
        print(f"\nEvaluating TRIs against ETM: {etm_name} on {graph_name}")
        ...
        common_eval_nodes = list(set(current_etm_scores.keys()).intersection(*[set(scores.keys()) for scores in tri_results.values()]))
        # Further filter for nodes where ETM score is not None or NaN
        common_eval_nodes = [n for n in common_eval_nodes if current_etm_scores.get(n) is not None and not np.isnan(current_etm_scores.get(n))]


        if not common_eval_nodes:
            print(f"  No common nodes with valid ETM scores for {etm_name}. Skipping evaluation for this ETM.")
            continue
            
        ordered_etm_for_eval = [current_etm_scores[n] for n in common_eval_nodes]

        for tri_name, current_tri_scores in tri_results.items():
            ordered_tri_for_eval = [current_tri_scores.get(n, 0.0) for n in common_eval_nodes] # Default to 0 if node missing in TRI

            # a. Spearman Correlation
            corr, p_val = eval_metrics.calculate_spearman_rank_correlation(ordered_tri_for_eval, ordered_etm_for_eval)
            print(f"  Spearman Corr ({tri_name} vs {etm_name}): {corr:.3f} (p={p_val:.3g})")
            
            current_eval_row = {'ETM': etm_name, 'TRI': tri_name, 'Spearman_Correlation': corr, 'P_value': p_val}
            ...
            for k_perc in config["evaluation_params"]["top_k_values_perc"]:
                ...
                pred_top_k = eval_metrics.get_top_k_nodes(current_tri_scores, k_perc, is_percentage=True)
                ...
                if not true_top_k: # Avoid issues if ETM scores are all same/zero
                    print(f"    Skipping P/R/F1 for k={k_perc*100:.0f}% as true_top_k is empty for {etm_name}.")
            ...
            #     {n: current_tri_scores.get(n,0.0) for n in common_eval_nodes},
    ...
    if config["save_results_csv"]:
        results_df.to_csv(os.path.join(config["results_dir"], f"{graph_name}_evaluation_summary.csv"), index=False)
    ...
    if config["save_plots"]:
        ...
        plotter.plot_degree_distribution(graph, graph_name, save_path=os.path.join(plot_dir, "degree_distribution.png"))

        # b. ETM Distributions
        for etm_name, scores in etm_scores.items():
            plotter.plot_etm_distribution(scores, etm_name, graph_name, save_path=os.path.join(plot_dir, f"dist_{etm_name}.png"))

        # c. Correlation Heatmap (requires results_df to be reshaped or a dedicated correlation matrix)
        # Reshape results_df for heatmap: index=TRI, columns=ETM, values=Spearman_Correlation
        try:
            corr_pivot_df = results_df.pivot(index='TRI', columns='ETM', values='Spearman_Correlation')
            plotter.plot_correlation_heatmap(corr_pivot_df, 
                                             title=f"Spearman Correlation: TRIs vs. ETMs ({graph_name})",
                                             save_path=os.path.join(plot_dir, "correlation_heatmap.png"))
        except Exception as e:
            print(f"Could not generate correlation heatmap: {e}")


        # d. Scatter/Rank Plots for a key ETM (e.g., MCSS) vs. a few key TRIs
        key_etm_for_scatter = 'MCSS' # Example
        if key_etm_for_scatter in etm_scores:
            key_tris_for_scatter = ['Degree (Out)', 'CI (l=2)'] # Add best proposed TRI name here once known
            if config["composite_wmfs_configs"]:
                key_tris_for_scatter.append(config["composite_wmfs_configs"][0]["name"]) # Add first WMFS-TRI

            for tri_name_sc in key_tris_for_scatter:
                if tri_name_sc in tri_results:
                    plotter.plot_scatter_rank_comparison(
                        tri_results[tri_name_sc], etm_scores[key_etm_for_scatter],
                        tri_name=tri_name_sc, etm_name=key_etm_for_scatter, graph_name=graph_name,
                        save_path_prefix=os.path.join(plot_dir, f"scatter_{safe_name(tri_name_sc)}_vs_{key_etm_for_scatter}")
                    )
        
        # e. Top-k Performance Bar Charts (e.g., F1@k for each ETM)
        # Reshape results_df for F1 scores: index=TRI, columns=k_value, values=F1
        for etm_name_bc in etm_scores.keys():
            f1_cols = [col for col in results_df.columns if 'F1@' in col and etm_name_bc in results_df[results_df['ETM'] == etm_name_bc]['ETM'].values]
            if f1_cols:
                f1_df_for_etm = results_df[results_df['ETM'] == etm_name_bc].set_index('TRI')[f1_cols]
                f1_df_for_etm.columns = [col.split('@')[1] for col in f1_cols] # Clean column names (e.g. "5%")
                if not f1_df_for_etm.empty:
                    plotter.plot_top_k_performance_bars(
                        f1_df_for_etm, metric_name="F1-Score@Top-k%", title_suffix=f"for {etm_name_bc} ({graph_name})",
...
def safe_name(name_str):
    return name_str.replace(" ", "_").replace("(", "").replace(")", "").replace("=","").replace("*","").replace(".","p").replace("%","perc")
...
if __name__ == '__main__':
    ...
    # e.g., parser.add_argument('--dataset_type', type=str, choices=['casflow', 'snap_twitter'])
