...
CASFLOW_DATA_PATH = "data_loader/dataset.txt"  # Example
...
def parse_casflow_data(file_path):
    ...
    try:
        with open(file_path, "r") as f:
            for line_number, line in enumerate(f):
                ...
                if len(parts) < 4:
                    print(
                        f"Warning: Skipping malformed line {line_number + 1}: {line} (not enough main parts)"
                    )
                    continue

                cascade_id_str, source_node_str, start_timestamp_str, num_infected_str = (
                    parts[0],
                    parts[1],
                    parts[2],
                    parts[3],
                ...
                try:
                    ...
                    print(
                        f"Warning: Skipping cascade {cascade_id_str} due to type conversion error in header: {e}"
                    )
                    continue
                ...
                for infection_event_str in infection_sequence_str:
                    try:
                        ...
                        if (
                            parent_node == infected_node
                        ):  # Or (infected_node == source_node and relative_infection_time == 0)
                        ...
                        if (
                            parent_node and infected_node and parent_node != infected_node
                        ):  # Ensure valid edge
                            interaction_edges.add((parent_node, infected_node))

                    except ValueError as e:
                        print(
                            f"Warning: Skipping malformed infection event '{infection_event_str}' in cascade {cascade_id}: {e}"
                        )
                        continue
                    except Exception as e:
                        print(
                            f"Warning: Unexpected error parsing infection event '{infection_event_str}' in cascade {cascade_id}: {e}"
                        )
                        continue
    ...
    print(
        f"Interaction graph created with {interaction_graph.number_of_nodes()} nodes and {interaction_graph.number_of_edges()} edges."
    )
